...
class ContestVideoClipViewController: UIViewController {
    
    // 当前正在播放的视频的信息
    var item: AVPlayerItem!
    var info: Contest.PublishInfo!
    ...
    class func instance() -> ContestVideoClipViewController {
        return StoryBoard.contest.instance()
    }

    class func instance(item: AVPlayerItem,
                        info: Contest.PublishInfo) -> ContestVideoClipViewController {
        let vc = instance()
        vc.item = item
        vc.info = info
        return vc
    ...
    deinit {
        ...
        removeObserver()
        print("ContestVideoClipViewController deinit")
    }
}

extension ContestVideoClipViewController {
    ...
    @IBAction func exportVideo(_ sender: UIButton) {
        ...
        clip(asset: item.asset, begin: beginTime, end: endTime) {
            ...
            if result, let url = this.videoURL {
                let vc = ContestPublishViewController.instance(
                    url: url,
                    info: this.info
                )
...
extension ContestVideoClipViewController {
    ...
    func getSingleTime() -> Double {
        let maxWidth = Double(collectionView.width - 60)
...
extension ContestVideoClipViewController {
    ...
    func loadThumbnails() {
        ...
        let height = Double(collectionView.height)
        ...
        let ratio = Double(fabs(size.width) / fabs(size.height))
        ...
        while frame < totalFrames {
            let time = CMTimeMake(Int64(frame), Int32(fps))
        ...
        generator.generateCGImagesAsynchronously(forTimes: times) {
            ...
            case .succeeded:
                ...
                DispatchQueue.main.async {
                    let image = UIImage(cgImage: cgImage).compressed()
...
extension ContestVideoClipViewController {
    ...
    private func clip(asset: AVAsset,
        ...
        let fps = asset.tracks(withMediaType: .video).last?.nominalFrameRate ?? 0
        let startDuration = CMTimeMakeWithSeconds(Float64(begin), Int32(fps))
        let duration = CMTimeMakeWithSeconds(Float64(end - begin), Int32(fps))
        ...
        do{
            try compositionVideoTrack?.insertTimeRange(
                CMTimeRangeMake(startDuration, duration),
                of: assetVideoTrack,
                at: kCMTimeZero
            )
...
extension ContestVideoClipViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withClass: ContestVideoClipCell.self, for: indexPath)
        cell.set(image: thumbnails[indexPath.item].image)
        return cell
    }
}

extension ContestVideoClipViewController: UICollectionViewDelegateFlowLayout {
